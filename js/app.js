document.addEventListener('DOMContentLoaded', () => {
    ...
    function removeSongFromPlaylist(playlistId, songFile) { const playlist = playlistsData[playlistId]; playlist.songs = playlist.songs.filter(sf => sf !== songFile); saveState(); renderPlaylistsList(); renderPlaylistDetail(playlistId); const song = songMap.get(songFile); showToast(`"${song.title}" removido da playlist`); }
    ...
    function changePlaylistCover(playlistId, file) {
        ...
        reader.onload = (e) => {
            image.onload = () => {
                ...
                playlistsData[playlistId].cover = canvas.toDataURL('image/jpeg', 0.8);
                saveState(); renderAll();
            };
    ...
    function updateRepeatButtonUI() { repeatBtn.classList.remove('active', 'repeat-mode-one'); if (state.repeatMode === 'all') repeatBtn.classList.add('active'); else if (state.repeatMode === 'one') repeatBtn.classList.add('active', 'repeat-mode-one'); }
    ...
    function setupEventListeners() {
        ...
        audio.addEventListener('ended', () => { const isLastSong = state.currentSongIndexInPlaylist === getActivePlaylistSongs().length - 1; if (state.repeatMode === 'one') { audio.currentTime = 0; play(); } else if (state.repeatMode === 'none' && !state.isShuffle && isLastSong) { pause(); } else { changeTrack('next'); } });
        ...
        effectsToggleBtn.addEventListener('click', () => { state.isEffectsEnabled = !state.isEffectsEnabled; connectAudioEffects(); saveState(); });
        ...
        playlistDetailSongs.addEventListener('click', (e) => {
            ...
            if (songInfo) {
                const songIndex = parseInt(songInfo.parentElement.dataset.songIndex);
                const playlistId = state.playlistIdToEdit;
                state.activePlaylistId = playlistId;
                loadSong(songIndex, true);
                switchToView('library-view');
                return;
            }
        ...
        searchResultsEl.addEventListener('click', (e) => {
            ...
            if (addBtn) {
                state.songFileToAddToPlaylist = addBtn.dataset.songFile;
    ...
    function initializeAudioContext() {
        ...
        compressorNode.threshold.setValueAtTime(-24, audioContext.currentTime); compressorNode.knee.setValueAtTime(30, audioContext.currentTime); compressorNode.ratio.setValueAtTime(12, audioContext.currentTime); compressorNode.attack.setValueAtTime(0.05, audioContext.currentTime); compressorNode.release.setValueAtTime(0.3, audioContext.currentTime);
    ...
    function connectAudioEffects() {
        ...
        if (state.isEffectsEnabled) {
            source.connect(compressorNode); let currentNode = compressorNode;
            eqBands.forEach(band => { currentNode.connect(band); currentNode = band; });
            currentNode.connect(audioContext.destination); effectsToggleBtn.classList.add('active');
        } else {
            source.connect(audioContext.destination); effectsToggleBtn.classList.remove('active');
        }
    }
    ...
    function updateVolumeUI() {
        ...
        if (state.isMuted || state.volume === 0) { volumeBtn.innerHTML = '<i class="fas fa-volume-xmark"></i>'; } else if (state.volume < 0.5) { volumeBtn.innerHTML = '<i class="fas fa-volume-low"></i>'; } else { volumeBtn.innerHTML = '<i class="fas fa-volume-high"></i>'; }
    }
